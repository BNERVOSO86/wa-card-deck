...
export function PhoneCard({
  ...
  const daysAgoLabel = (() => {
    if (lastRecharge === "Sem recarga" || daysSinceRecharge === null) return null;
    return `${daysSinceRecharge} ${daysSinceRecharge === 1 ? 'dia' : 'dias'} atrás`;
  })();
  ...
  return (
    <Card className={`group hover:border-primary/50 transition-all duration-300 ${getBorderClass()} ${getCardBgClass()}`}>
      ...
      <CardHeader className="flex flex-row items-center justify-center pb-3 relative">
      ...
      <CardContent className="space-y-4">
        ...
        <div className="pt-2 border-t border-border">
          <p className="text-xs text-muted-foreground mb-1 text-center">Webhook:</p>
          <p className="text-xs font-mono bg-secondary px-2 py-1 rounded truncate text-center">{webhook}</p>
        </div>
